...
exports.registerPrivacyTools = registerPrivacyTools;
const zod_1 = require("zod");
const api_js_1 = require("../services/api.js");
// RoPA fields match lib/rpa/index.ts field definitions
function registerPrivacyTools(server) {
    // ── Get RoPA ──────────────────────────────────────────────────────────────
    server.registerTool("unicis_get_ropa", {
...
Returns: RoPA procedure data (multi-step object) or indication it is not set.`,
        ...
        return {
            ...
            structuredContent: { data: rpa },
        };
    });
    // ── Set RoPA ──────────────────────────────────────────────────────────────
    server.registerTool("unicis_set_ropa", {
...
Returns: Confirmation.`,
        inputSchema: zod_1.z.object({
            slug: zod_1.z.string().describe("Team slug"),
            taskNumber: zod_1.z.number().int().positive().describe("Task number"),
            procedure: zod_1.z.array(zod_1.z.record(zod_1.z.unknown())).min(1).describe("6-step RoPA procedure array"),
        }).strict(),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ slug, taskNumber, procedure }) => {
        const task = await (0, api_js_1.apiGet)(`/api/teams/${slug}/tasks/${taskNumber}`);
        const prevProcedure = task.properties?.rpa_procedure ?? [];
        await (0, api_js_1.apiPost)(`/api/teams/${slug}/tasks/${taskNumber}/rpa`, { prevProcedure, nextProcedure: procedure });
        return {
            content: [{ type: "text", text: `✅ RoPA saved on task #${taskNumber}.` }],
        };
    });
    // ── Delete RoPA ───────────────────────────────────────────────────────────
    server.registerTool("unicis_delete_ropa", {
...
Returns: Confirmation.`,
        ...
        await (0, api_js_1.apiDelete)(`/api/teams/${slug}/tasks/${taskNumber}/rpa`);
        return {
            content: [{ type: "text", text: `✅ RoPA removed from task #${taskNumber}.` }],
        };
    });
    // ── Get TIA ───────────────────────────────────────────────────────────────
    server.registerTool("unicis_get_tia", {
...
Returns: TIA procedure data or indication it is not set.`,
        ...
        return {
            ...
            structuredContent: { data: tia },
        };
    });
    // ── Set TIA ───────────────────────────────────────────────────────────────
    server.registerTool("unicis_set_tia", {
...
Returns: Confirmation.`,
        inputSchema: zod_1.z.object({
            slug: zod_1.z.string().describe("Team slug"),
            taskNumber: zod_1.z.number().int().positive().describe("Task number"),
            procedure: zod_1.z.array(zod_1.z.record(zod_1.z.unknown())).min(1).describe("TIA procedure steps array"),
        }).strict(),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ slug, taskNumber, procedure }) => {
        const task = await (0, api_js_1.apiGet)(`/api/teams/${slug}/tasks/${taskNumber}`);
        const prevProcedure = task.properties?.tia_procedure ?? [];
        await (0, api_js_1.apiPost)(`/api/teams/${slug}/tasks/${taskNumber}/tia`, { prevProcedure, nextProcedure: procedure });
        return {
            content: [{ type: "text", text: `✅ TIA saved on task #${taskNumber}.` }],
        };
    });
    // ── Delete TIA ────────────────────────────────────────────────────────────
    server.registerTool("unicis_delete_tia", {
...
Returns: Confirmation.`,
        ...
        await (0, api_js_1.apiDelete)(`/api/teams/${slug}/tasks/${taskNumber}/tia`);
        return {
            content: [{ type: "text", text: `✅ TIA removed from task #${taskNumber}.` }],
        };
    });
    // ── Get PIA ───────────────────────────────────────────────────────────────
    server.registerTool("unicis_get_pia", {
...
Returns: Confirmation.`,
    ...
    // ── Delete PIA ────────────────────────────────────────────────────────────
    server.registerTool("unicis_delete_pia", {
...
Returns: Confirmation.`,
        ...
        await (0, api_js_1.apiDelete)(`/api/teams/${slug}/tasks/${taskNumber}/pia`);
        return {
